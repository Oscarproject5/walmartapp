...
ADD COLUMN IF NOT EXISTS sold_stock_value DECIMAL(10, 2) DEFAULT 0,
...
SET 
    ...
    sold_stock_value = COALESCE(sales_qty, 0) * cost_per_item,
    supplier = source,
    status = 'active';
...
BEGIN
    ...
    NEW.sold_stock_value := COALESCE(NEW.sales_qty, 0) * NEW.cost_per_item;
    
    -- Update status based on available quantity
    IF NEW.available_qty <= 0 THEN
        NEW.status := 'out_of_stock';
    ELSIF NEW.available_qty < 5 THEN
...
BEGIN
    ...
    IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.product_id = NEW.product_id) THEN
        ...
        SET 
            ...
            sold_stock_value = cost_per_item * COALESCE((
                SELECT SUM(quantity_sold)
                FROM public.sales
                WHERE product_id = NEW.product_id
                AND status = 'active'
