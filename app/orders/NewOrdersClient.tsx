...
export default function NewOrdersClient() {
  ...
  // Function to create missing products and continue with import
  const createMissingProducts = async (skus: string[]) => {
    ...
    try {
      ...
      // If some SKUs need to be created, create them
      if (skusToCreate.length > 0) {
        ...
        // Update the existing SKU map with newly created products
        skusToCreate.forEach(sku => {
          existingSkuMap.set(sku.toLowerCase(), sku);
        });
      } else {
        console.log('All SKUs already exist in the database');
      }

      // Update the mapped data with correct SKU cases
  ...
  const processMappedData = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        const { data: userData } = await supabase.auth.getUser(); // Ensure we have user ID
        const userId = userData?.user?.id;

        if (!userId) {
          console.error("Cannot refresh orders: User ID not found after import.");
